...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const currentThemeColor =
        localStorage.getItem("currentThemeColor") || defaultThemeColor.label;
  ...
  async function handleContentDelete(content: ContentWithId) {
    try {
      const allDeletedContents = contents.filter(
        (item) => item.parentId === content.id
      );
      allDeletedContents.push(content);
      const allDeletedContentIds = allDeletedContents.map((item) => item.id);
      setContents(
        contents.filter((item) => !allDeletedContentIds.includes(item.id))
      );
      ...
      toast({
        ...
        action: (
          <ToastAction
            altText="undo"
            onClick={() => handeleContentsUndo(allDeletedContents)}
          >
            撤销
          </ToastAction>
        ),
      ...
      <main className="h-[calc(100%-58px)]">
        ...
        >
          ...
          <TabsContent
            value="preview"
            forceMount
            className="data-[state=inactive]:hidden sm:!block flex-grow sm:flex-grow-0 flex-shrink sm:flex-shrink-0 overflow-y-auto mt-0"
          >
            <div
              className={cn(
