...
export default function EditorImage(props: EditorFooterProps) {
  ...
  return (
    <div className="editor-image">
      {imageFiles.length > 0 && (
        <ul className="upload-preview grid grid-cols-4 gap-2 mb-6">
          {imageFiles.map((item) => (
            <li
              key={item.uid || item.name}
              className="h-[120px] group flex items-center gap-2 relative"
            >
              <Image
                src={item.dataUrl}
                alt={item.name}
              ...
              />
              <div
                className="flex items-center justify-center absolute right-1 top-1 w-4 h-4 bg-gray-700 rounded-full"
