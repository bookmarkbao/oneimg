...
const PreviewItem = forwardRef<
  HTMLLIElement,
  { content: Content; children?: React.ReactNode; index: number; theme: string }
>(({ content, children, index, theme }, ref) => {
  ...
  return (
    ...
    >
      ...
      {(stripEmptyParagraphs(content.content) ||
        imageFiles.length > 0 ||
        children) && (
        <div
          className={cn(
            content.parentId
              ? "one-child-item__content"
              : content.type === "theme_content"
              ? "one-theme__content"
              : "one-item__content"
          )}
        >
          ...
          {imageFiles.length > 0 && (
            <ImageList
              images={imageFiles}
              gridLayout={getThemeBaseClass(theme) === "red-post"}
            />
          )}
          {children}
        </div>
