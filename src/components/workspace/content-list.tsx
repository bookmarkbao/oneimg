...
export default function ContentList(props: ContentListProps) {
  ...
  const [curContent, setCurContent] = useState<ContentWithId>(
    {} as ContentWithId
  );
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (
      overItem.type === "theme_content" ||
      activeItem.type === "theme_content"
    ) {
    ...
    function resortContents(items: ContentWithId[]) {
      ...
      const bothAtRootLevel =
        activeItem.parentId == null && overItem.parentId == null;

      // Check if the active item is a child and the over item is a parent
      const movingChildToParent =
        typeof activeItem.parentId === "number" && overItem.parentId == null;
      ...
      if (bothAtRootLevel) {
        ...
        return newItems.map((item, index) =>
          item.parentId == null ? { ...item, childOrder: index } : item
        );
      } else if (movingChildToParent) {
        // If moving a child to a parent, allow it
        ...
        const newSiblings = newItems.filter(
          (item) => item.parentId === overItem.id
        );
        ...
        const oldSiblings = newItems.filter(
        ...
        // If dragging within the same parent container
        newItems = arrayMove(newItems, activeIndex, overIndex);
              ...
              <div className="flex">
                ...
                {DOMPurify && typeof DOMPurify.sanitize === "function" && (
                  <div className="font-bold">
                    {parse(DOMPurify.sanitize(curContent.title))}
                  </div>
                )}
        ...
        {activeId &&
          (() => {
            const activeContent = contents.find(
              (contentItem) => contentItem.id === activeId
            );
            if (!activeContent) {
              return null;
            }
