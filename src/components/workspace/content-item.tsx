    ...
    <SortableItem item={item} disabled={disabled}>
      {editingContentId === item.id ? (
        ...
        >
          ...
          <a
            href={`/#${item.id}`}
            className="relative border-b border-b-border py-4 hidden sm:block"
          >
          ...
          {/* buttons */}
          <div
            className="relative border-b border-b-border py-4 sm:hidden"
            onClick={(e) => e.preventDefault()}
          >
            <div className="mr-28 select-none">
              {parse(DOMPurify.sanitize(item.title))}
            </div>
            <ContentItemButtons
              item={item}
              onContentEdit={handleContentEdit}
